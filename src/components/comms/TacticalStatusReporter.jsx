...
export default function TacticalStatusReporter({ user, eventId }) {
  ...
  const { data: latestLog } = useQuery({
    ...
    queryFn: async () => {
       const logs = await dataClient.entities.AIAgentLog.list({
          filter: { 
             event_id: eventId,
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      const { data: result } = await dataClient.functions.invoke('inferTacticalStatus', {
          report: report,
          userRank: user?.rank || 'Unknown'
      });

      // Broadcast result via AIAgentLog
      await dataClient.entities.AIAgentLog.create({
         event_id: eventId,
         agent_slug: 'tactical-computer',
         type: 'INFO',
