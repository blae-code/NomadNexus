...
export default function CommsDashboardPanel({ user, eventId, className }) {
  ...
  const { room, connectionState, audioState, connectionMetrics } = useLiveKit();
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-zinc-500 text-[9px]">
                          ...
                          {canManageCampfires && (
                            <button
                              type="button"
                              className="ml-2 px-2 py-0.5 border border-red-800/70 text-red-300 hover:bg-red-900/30 rounded"
                              onClick={(e) => {
                                e.stopPropagation();
