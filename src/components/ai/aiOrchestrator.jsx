...
async function createFinding(eventId, agentSlug, type, severity, summary, details) {
  return supabaseApi.entities.AIAgentLog.create({
    event_id: eventId,
...
async function runCommsWatch(eventId) {
  ...
  const statuses = await supabaseApi.entities.PlayerStatus.list({ event_id: eventId });
...
async function executeAction(actionType, actionParamsStr, eventId) {
  ...
  if (actionType === 'AUTO_ASSIGN_ROLE') {
    ...
    const candidates = await supabaseApi.entities.PlayerStatus.list({ 
    ...
    if (candidates.length > 0) {
      ...
      await supabaseApi.entities.PlayerStatus.update(target.id, { role: role });
...
async function runOpsMonitor(eventId) {
  // Fetch Data
  const statuses = await supabaseApi.entities.PlayerStatus.list({ event_id: eventId });
  const event = await supabaseApi.entities.Event.get(eventId);
  ...
  const rules = await supabaseApi.entities.AIAgentRule.list({ filter: { agent_slug: 'ops-monitor', is_active: true } });
...
export async function refreshAgent(agentSlug, eventId) {
  ...
  // await supabaseApi.entities.AIAgentLog.deleteMany({ event_id: eventId, agent_slug: agentSlug }); // Not available in SDK yet usually, just append new ones.
